...
function stop_bidding(uint _request_no) public OnlyLA returns (string)
{
    ...
    wallet[_borrower].balance=wallet[_borrower].balance+  (_la_share*100)*_req_money;
    wallet[msg.sender].balance=wallet[msg.sender].balance -   (_la_share*100)*_req_money;
...
function rePayment(uint request_no, uint amount, string _installmentCount) public{
//to be done
}
